...
export default function ServiceDetailClient({ service }: { service: any }) {
  ...
  return (
    <>
      <section className="relative overflow-hidden px-4 sm:px-6 lg:px-8" style={{ backgroundColor: "#fefce8", paddingTop: "3.5rem", paddingBottom: "5rem" }}>
        <PawIcon size={128} className="absolute top-3 right-[7%] text-[var(--color-primary)] hidden md:block" style={{ opacity: 0.06 }} />
        <div className="relative max-w-4xl mx-auto">
          ...
          <h1 className="mb-3" style={{ fontFamily: "var(--font-heading)", fontSize: "clamp(2.25rem, 5vw, 3rem)", fontWeight: 800, color: "var(--color-text-primary)" }}>{service.name}</h1>
                ...
                {derivedVariant && (
                  <div className="pt-6 space-y-4" style={{ borderTop: "1px solid var(--color-border)" }}>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-bold" style={{ color: "var(--color-text-primary)" }}>{formatPrice(unitPrice)}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2">
                      ...
                      <span className="text-2xl font-bold" style={{ color: "var(--color-text-primary)" }}>{formatPrice(subtotal)}</span>
                    </div>
                    <div className="flex flex-col gap-3 pt-2">
                       <button onClick={handleAddToCart} className="flex items-center justify-center gap-2 w-full px-6 py-3.5 font-semibold text-sm transition-all hover:-translate-y-0.5"
                        style={{ borderRadius: "2rem", backgroundColor: "var(--color-secondary)", color: "var(--color-text-primary)", boxShadow: "0 4px 16px rgba(239,242,58,0.35)" }}>
