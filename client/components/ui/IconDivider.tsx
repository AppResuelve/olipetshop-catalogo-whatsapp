...
function IconLayer({ path, color, patternId, cycleW, iconX, iconSize, height, yOffset = 0 }: any) {
  return (
    <svg className="absolute inset-0 w-full" height={height} preserveAspectRatio="none">
      <defs>
        <pattern id={patternId} width={cycleW} height={height} patternUnits="userSpaceOnUse">
          <svg viewBox="0 0 24 24" x={iconX} y={(height - iconSize) / 2 + yOffset} width={iconSize} height={iconSize} fill={color}>
            <path d={path} />
          </svg>
        </pattern>
      </defs>
...
export function IconDivider({ className = "", size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  const uid = useId()
  ...
  return (
    <div className={`w-full pointer-events-none ${className}`}>
      <div className="relative w-full" style={{ height: cfg.height }}>
        <IconLayer path={BONE} color="rgba(239,242,58,0.15)" patternId={`${uid}-b`} cycleW={cfg.cycleW} iconX={4} iconSize={cfg.iconSize} height={cfg.height} yOffset={-5} />
        <IconLayer path={CAT} color="rgba(239,242,58,0.25)" patternId={`${uid}-c`} cycleW={cfg.cycleW} iconX={midX} iconSize={cfg.iconSize} height={cfg.height} yOffset={0} />
        <IconLayer path={HOME} color="rgba(239,242,58,0.35)" patternId={`${uid}-h`} cycleW={cfg.cycleW} iconX={cfg.cycleW - cfg.iconSize - 4} iconSize={cfg.iconSize} height={cfg.height} yOffset={5} />
      </div>
    </div>
  )
}
